        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转，直接从a路由跳转到b路由，而这里的进入购物车在路由跳转之前还做了一件事情就是发请求
...
export default {
  ...
  methods: {
    ...
    changeActive(a, arr) {
      ...
      arr.forEach(element => {
        element.isChecked = '0'  //我真是个二百五 三等是表判断 不是赋值
      })
      a.isChecked = '1'
    },
    // 表单元素修改产品个数
    changeSkuNum(skuNum) {
      // 这里也可也不把上面的这个参数传入 直接用e.target.value也可拿到这个值
      // isNaN() 应该是一个方法 判断参数是不是非法 
    ...
    async addShopcar() {
      ...
      //2: 此处是派发action 需要判断加入购物车成功还是失败，再进行相应操作 并且仓库在对象派发的那个函数前面加了async 返回值自动包装成Promise类型然后返回给你
      // 返回值是仓库里面写的 return 'ok' 也就是返回一个promise，具体参数是[[PromiseState]]:"fulfilled" ,[[PromiseResult]]:"ok" 
      //   加了async await 那么就会直接获得promise成功回调结果ok，try和catch好像就是成功执行try里面 失败执行catch里面函数  
      try {
        // 如果成功了 
        let result = await this.$store.dispatch('detail/addOrUpdateShopCart', { skuId: this.$route.params.skuid, skuNum: this.skuNum })
        // console.log(result)
        //3: 进行路由跳转  4:并且把产品信息带给下一级路由组件
        // 一些简单的数据skuNum，直接通过query形式给路由组件传递，比较复杂的数据如skuInfo就是那种对象里面有数组啥的 复杂的，通过会话存储(不持久化，会话结束消失)
        sessionStorage.setItem('SKUINFO', JSON.stringify(this.skuInfo))
        // this.$bus.$emit('skuInfo', this.skuInfo)
