<template>
  <div class="clearfix selector">
    ...
    <!-- 平台售卖属性 是一个数组里面有多个对象 然后多个对象对象里面分别有几个属性 其中一个属性是数组 也可以遍历横向生成数据-->
    <div class="type-wrap" v-for="(attr, index) in attrsList" :key="attr.attrId">
      <!-- 这里是手机系统，运行内存，机身内存那一条竖着显示的数据 -->
...
export default {
  ...
  methods: {
    ...
    tradeMarkHandler(trademark) {
      // 点击了品牌 子组件整理参数，传递给header父组件
      // 子给父传数据  自定义事件
      this.$emit('trademarkInfo', trademark)
    },
    // 平台售卖属性值的点击事件  自定义事件
    attrInfo(attr, attrValue) {
      // 这里麻烦的特地取出attr里面嵌套的数组attrvalue的原因是这个数组需要遍历出来 不然通过attr不能知道具体点击的是那一个
      // ['属性ID:属性值：属性名']
