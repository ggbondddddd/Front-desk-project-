<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(ad, index) in address" :key="ad.id">
...
export default {
  ...
  computed: {
    ...
    // 将来提交订单最终选中地址
    userDefaultAddress() {
      // find方法：查找数组中符合条件的元素返回，为最终结果 {{ userDefaultAddress.fullAddress }}
      // filter方法：查找数组中符合条件的元素放到数组里面返回 所以如果用filter方法做这个地方就是{{ userDefaultAddress[0].fullAddress }}
  ...
  methods: {
    ...
    // 提交订单   注意这里经常会显示null 很有可能是你提交了重复的订单
    async submitOrder() {
      // 交易编码
      ...
      if (result.code === 200) {
        ...
        //  路由跳转加路由传参 把提交的订单的订单编号传递给下一级  也可直接键值对传参this.$router.push('/pay?orderId =' + this.orderId)
